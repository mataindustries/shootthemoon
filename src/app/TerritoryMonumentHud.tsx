...
export type MonumentAction = MonumentKind | MonumentOrder | 'repair'
  ...
  return <section className={`monument-panel${reveal ? ' monument-panel--reveal' : ''}`} data-status={m?.status ?? 'choices'} aria-label="Territory Monuments">
    ...
    {m === null ? <>
      ...
      {reveal ? <p role="status">TERRITORY CLAIMED · {spec!.benefit}</p> : <>
        ...
        {m.status !== 'complete' && m.status !== 'damaged' ? <p className="monument-resources">
        ...
        {(m.status === 'command' || m.status === 'wave') && wave ? <div className="monument-command">
          ...
          <p role="status"><strong>WAVE {m.wavesResolved + 1} / 3 · {wave.name}</strong><br />APPROACH: {wave.approach}{m.status === 'wave' ? ` · ${Math.ceil((wave.durationMs - m.phaseElapsedMs) / 1000)}s` : ''}</p>
          {m.status === 'command' ? <>
            <p>COMMAND PHASE · Choose one allocation. Attack waits for your order.</p>
          ...
          </> : <p>{MONUMENT_ORDERS[m.orders[m.wavesResolved]!].title} · ALLOCATION LOCKED</p>}
        </div> : null}
