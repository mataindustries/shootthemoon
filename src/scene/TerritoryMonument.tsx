...
export function TerritoryMonument({ monument, site, terrain, segments, onFocus }: {
  ...
  const wave = OCTOGONALS.waves[monument.wavesResolved]
  ...
  return <group position={transform.position} quaternion={transform.orientation} name="territory-monument" onClick={onClick} dispose={null}>
    ...
    {(monument.status === 'command' || monument.status === 'wave') && wave ? <group name="octogonal-approach">
      <OctagonalFleet batches={fleet} kit={kit} poses={[0, 1, 2].map(ship => ({
        position: octogonalApproach(monument.wavesResolved, monument.phaseElapsedMs / wave.durationMs, ship).toArray(),
        heading: Math.atan2(wave.origin[0], wave.origin[2]),
        bank: (ship - 1) * .12,
        scale: ship === 1 ? .007 : .0058,
      }))} />
      {[.15, .35, .55, .75].map(p => <mesh key={p} position={octogonalApproach(monument.wavesResolved, p, 1)}
        geometry={kit.shapes.bevel} material={kit.materials.cyan} scale={[.00065, .0005, .00065]} />)}
    </group> : null}
  </group>
}
